...
int main(int argc, char **argv) {
        const char *fichier = (argc > 1) ? argv[1] : "fic_test/fic_test_err_dec.txt";
        ...
        /* 1) Sauvegarde */
        if (sauvegarder("etat.bin") == 0) {
                printf("Sauvegarde OK -> etat.bin\n");
        } else {
                printf("ERREUR: sauvegarde impossible\n");
        }

        /* 2) Reset des tables (pour vérifier que le chargement marche) */
        init_tab_lexico();
        init_tab_decl();
        init_tab_repres();
        init_tab_region();

        /* (optionnel) reset aussi la pile si tu veux vraiment repartir propre */
        /* pile_region = creationPile();
           numregion = 0;
        ...
        */
        ...
        /* 3) Chargement */
        if (charge("etat.bin") == 0) {
                printf("Chargement OK <- etat.bin\n");
        } else {
                printf("ERREUR: chargement impossible\n");
        }

        /* 4) Affichage après chargement */
        printf("\n========================================\n");
        printf("        TABLES APRES CHARGEMENT\n");
        printf("========================================\n");
        afficher_tab_lex(NULL);
        afficher_tab_decl(stdout);
        afficher_tab_repres();
