...
int sauvegarde_tab_region(FILE *f)
{
    if (!f) return -1;

    /* on sauvegarde le nombre de régions */
    if (fwrite(&nb_regions, sizeof(nb_regions), 1, f) != 1)
        return -1;

    /* on sauvegarde NIS + taille pour chaque région */
    for (int i = 0; i < nb_regions; i++) {
        if (fwrite(&tab_region[i].NIS, sizeof(tab_region[i].NIS), 1, f) != 1)
            return -1;
        if (fwrite(&tab_region[i].taille, sizeof(tab_region[i].taille), 1, f) != 1)
            return -1;
    }
...
int charger_tab_region(FILE *f)
{
    ...
    if (n < 0 || n > TAB_REGION_MAX)
        return -1;

    nb_regions = n;

    for (int i = 0; i < nb_regions; i++) {
        if (fread(&tab_region[i].NIS, sizeof(tab_region[i].NIS), 1, f) != 1)
            return -1;
        if (fread(&tab_region[i].taille, sizeof(tab_region[i].taille), 1, f) != 1)
            return -1;

        tab_region[i].arbre = NULL;  /* pointeur non sauvegardé */
    }
