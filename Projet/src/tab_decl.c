//Ce module a été réaliser par ANAS et YACCINE 
#include <stdio.h>
#include <stdlib.h>
...
int sauvegarde_tab_decl(FILE *f)
{
    if (!f) return -1;

    if (fwrite(&zone_de_deb_utiliser, sizeof(zone_de_deb_utiliser), 1, f) != 1)
        return -1;

    if (fwrite(tab_de_dec, sizeof(TAB_DE_Decl), Taille_TAB, f) != (size_t)Taille_TAB)
        return -1;

    return 0;
}

int charger_tab_decl(FILE *f)
{
    if (!f) return -1;

    if (fread(&zone_de_deb_utiliser, sizeof(zone_de_deb_utiliser), 1, f) != 1)
        return -1;

    if (fread(tab_de_dec, sizeof(TAB_DE_Decl), Taille_TAB, f) != (size_t)Taille_TAB)
        return -1;
