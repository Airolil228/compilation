    ...
    int getNbLexico(char *lexeme){
        for(int i = 0; i < MAX_TAB_LEXICO; i++){
            if(strcmp(tab_lexico[i].lexeme, lexeme) == 0){
                return i; 
            }
        }
        return -1;
...
int sauvegarde_tab_lexico(FILE *f)
{
    if (!f) return -1;

    if (fwrite(&premier_el_vide_tablexico, sizeof(premier_el_vide_tablexico), 1, f) != 1)
        return -1;

    if (fwrite(tab_hashcode, sizeof(tab_hashcode[0]), MAX_TAB_HASHCODE, f) != (size_t)MAX_TAB_HASHCODE)
        return -1;

    if (fwrite(tab_lexico, sizeof(tab_lexico[0]), MAX_TAB_LEXICO, f) != (size_t)MAX_TAB_LEXICO)
        return -1;

    return 0;
}

int charger_tab_lexico(FILE *f)
{
    if (!f) return -1;

    if (fread(&premier_el_vide_tablexico, sizeof(premier_el_vide_tablexico), 1, f) != 1)
        return -1;

    if (fread(tab_hashcode, sizeof(tab_hashcode[0]), MAX_TAB_HASHCODE, f) != (size_t)MAX_TAB_HASHCODE)
        return -1;

    if (fread(tab_lexico, sizeof(tab_lexico[0]), MAX_TAB_LEXICO, f) != (size_t)MAX_TAB_LEXICO)
        return -1;
