%{
        ...
        {
        /* 1. Déclarer la procédure AVANT d'entrer dans sa région */
                sem_decl_proc($2, 0);
    
        /* 2. Créer et entrer dans la nouvelle région */
                empile(pile_region, numregion);
                numregion++;
    
                printf("[Parser] Entrée PROCEDURE '%s' région %d\n", 
                tab_lexico[$2].lexeme, numregion);
        } liste_parametres corps {
         /* 3. Sortir de la région */
                printf("[Parser] Sortie PROCEDURE région %d\n", numregion);
                depile(pile_region);
        ...
        {
        /* 1. Créer et entrer dans la nouvelle région */
                empile(pile_region, numregion);
                numregion++;
    
                printf("[Parser] Entrée FONCTION '%s' région %d\n", 
                tab_lexico[$2].lexeme, numregion);
        } liste_parametres RETOURNE type_simple {
        /* 2. Déclarer la fonction avec son type de retour (région parente) */
                depile(pile_region);  // Retour temporaire à la région parente
                sem_decl_fct($2, $6, 0);
                empile(pile_region, numregion);  // Re-entrer dans la région
        } corps {
        /* 3. Sortir de la région */
                printf("[Parser] Sortie FONCTION région %d\n", numregion);
                depile(pile_region);
